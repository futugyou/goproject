...
var oauth_client_request_info_db_name string = "oauth_client"
var oauth_client_request_info_table_name string = "oauth_request"
...
func NewAuthMongoDBClient(opts AuthOptions) *AuthMongoDBClient {
	...
	if opts.RequestTable == nil {
		opts.RequestTable = &oauth_client_request_info_table_name
	}
...
func (a *AuthMongoDBClient) RedirectToAuthorizationEndPoint(w http.ResponseWriter, r *http.Request) {
	authCodeURL := a.createAuthCodeURL(r, a.OauthConfig)
	http.Redirect(w, r, authCodeURL, http.StatusFound)
}

func (a *AuthMongoDBClient) Oauth2(w http.ResponseWriter, r *http.Request) {
	r.ParseForm()
...
func (a *AuthMongoDBClient) VerifyToken(w http.ResponseWriter, r *http.Request, token *oauth2.Token) bool {
...
func (a *AuthMongoDBClient) saveToken(ctx context.Context, token *oauth2.Token) error {
	...
	coll := a.Client.Database(*a.Options.DbName).Collection(*a.Options.RequestTable)
	_, err := coll.InsertOne(ctx, model)
	return err
}

func (a *AuthMongoDBClient) getAuthRequestInfo(ctx context.Context, state string) AuthModel {
	var model AuthModel
	coll := a.Client.Database(*a.Options.DbName).Collection(*a.Options.RequestTable)
...
func (a *AuthMongoDBClient) createAuthCodeURL(r *http.Request, config oauth2.Config) string {
