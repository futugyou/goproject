...
func Init() error {
	...
	if _, err := os.Stat(common.DefaultNetworkPath); err != nil {
		if os.IsNotExist(err) {
			_ = os.MkdirAll(common.DefaultNetworkPath, 0644)
		} else {
			return err
		}
	}

	_ = filepath.Walk(common.DefaultNetworkPath, func(nwPath string, info os.FileInfo, err error) error {
...
func enterContainerNetns(enLink *netlink.Link, cinfo *container.ContainerInfo, ep *Endpoint) func() {
	logrus.Infof("enterContainerNetns: %s", cinfo.Pid)
...
func configEndpointIpAddressAndRoute(ep *Endpoint, cinfo *container.ContainerInfo) error {
	...
	logrus.Infof("peerLink index: %d -- %s", peerLink.Attrs().Index, peerLink.Attrs().Name)
...
func configPortMapping(ep *Endpoint, cinfo *container.ContainerInfo) error {
	logrus.Infof("端口映射：%s, %s", ep.IpAddress, ep.PortMapping)
	// 遍历容器端口映射列表
...
func Connect(networkName string, cinfo *container.ContainerInfo) error {
	...
	if err = drivers[network.Driver].Connect(network, ep); err != nil {
		logrus.Errorf("connet err: %v", err)
		return err
	}

	// 配置端口映射
	if err = configPortMapping(ep, cinfo); err != nil {
		logrus.Errorf("config port mapping: %v", err)
		return err
	}

	// 到容器的namespace配置容器的网络设备IP地址
	if err = configEndpointIpAddressAndRoute(ep, cinfo); err != nil {
		logrus.Errorf("configEndpointIpAddressAndRoute err: %v", err)
		return err
	}
...
func CreateNetwork(driver, subnet, name string) error {
	...
	logrus.Infof("create network success")
	return nw.dump(common.DefaultNetworkPath)
}
