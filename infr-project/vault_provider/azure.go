...
func (s *AzureClient) SearchAsync(ctx context.Context, key string) (<-chan *ProviderVault, <-chan error) {
...
func (s *AzureClient) PrefixSearchAsync(ctx context.Context, prefix string) (<-chan map[string]ProviderVault, <-chan error) {
...
func (s *AzureClient) BatchSearchAsync(ctx context.Context, keys []string) (<-chan map[string]ProviderVault, <-chan error) {
	resultChan := make(chan map[string]ProviderVault, 1)
	errorChan := make(chan error, 1)
	go func() {
		defer close(resultChan)
...
func (s *AzureClient) UpsertAsync(ctx context.Context, key string, value string) (<-chan *ProviderVault, <-chan error) {
	resultChan := make(chan *ProviderVault, 1)
	errorChan := make(chan error, 1)
	go func() {
		defer close(resultChan)
...
func (s *AzureClient) DeleteAsync(ctx context.Context, key string) <-chan error {
	errorChan := make(chan error, 1)
	go func() {
		defer close(errorChan)
